...
# Ordena los pantalones por marca y luego por precio de forma descendiente.
def sortPants(pants):
  if type(pants) != list:
    return print("Error, tipo de dato invalido")
  return sorted(
    pants,
...
def getCandidates(pants):
  if type(pants) != list:
    return "No es el tipo de dato esperado"
  ...
  for p in pants:
    # Si la marca del pantalón actual es diferente al anterior, significa que fue añadido 
    # a la lista el último pantalón de una marca posible, entonces lo eliminamos de la lista 
    # para que mantenga por lo menos un pantalón posible de esa marca...
...
if __name__ == "__main__":
  ...
  myData = getData(recibeData())

  # 2 .- Ordena los datos por marca y precio de forma descendiente
